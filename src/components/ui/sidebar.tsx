...
function SidebarGroupLabel({ className, asChild = false, ...props }: React.ComponentProps<'div'> & { asChild?: boolean }) {
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground/60 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-semibold uppercase tracking-[.14em] outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-3.5 [&>svg]:shrink-0',
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<'div'> & { showIcon?: boolean }) {
  // Fixed width, not randomized: this app has no async-loading sidebar today, and
  // Math.random() in render trips the React Compiler's purity check. Pick a
  // per-row width via props if varied placeholder widths are ever needed.
  return (
