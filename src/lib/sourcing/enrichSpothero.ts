...
/** Fetch+parse+upsert one facility, then stamp enrichedAt. Fails closed — never throws. */
async function enrichOne(record: SourcedParkingLocation): Promise<{ ok: true } | { ok: false; error: string }> {
  if (!record.sourceListingId) {
    ...
    return { ok: false, error: `${record.id}: no sourceListingId on a spothero record` };
  }
  try {
    const res = await fetchWithRetry(record.sourceUrl);
    ...
    const input = mapFacilityDetailToInput(detail, record.sourceUrl, record.sourceListingId);
    const doc = await upsertSourcedLocation(input);
    await markSourcedLocationEnriched(doc.id, new Date().toISOString());
    return { ok: true };
  } catch (err) {
    return { ok: false, error: `${record.sourceUrl}: ${err instanceof Error ? err.message : String(err)}` };
  }
}
...
export async function enrichSpotHeroFacilities(
  ...
  // for a Firestore `where('source','==','spothero').where('enrichedAt','==',null)`
  // composite-indexed query if this collection grows much larger.
  const all = await listSourcedLocations();
  const allSpothero = all.filter((r) => r.source === 'spothero');
  const skipped = allSpothero.filter((r) => r.enrichedAt).length;
  ...
  for (let i = 0; i < candidates.length; i += CONCURRENCY) {
    ...
    results.forEach((result, idx) => {
      if (result.status === 'rejected') {
        ...
        errors.push(`${chunk[idx].sourceUrl}: unexpected failure: ${msg}`);
